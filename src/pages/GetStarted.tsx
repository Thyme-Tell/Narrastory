    ...
    <div className="min-h-screen bg-[#EFF1E9] px-[7%]">
      <header className="py-4 px-4 sm:px-8 bg-transparent sticky top-0 z-50">
        <nav className="flex flex-col justify-between items-center bg-transparent py-1.5 sm:py-2 navbar-below-logo">
          <Link 
            to="/get-started" 
            className="bg-[#EFF1E9]/50 backdrop-blur-[100px] rounded-[100px] p-4 inline-block w-full sm:w-auto flex justify-center"
          ...
          <div className="flex w-full justify-center mt-4 navbar-menu">
            <div className="hidden sm:flex bg-[#8A9096]/40 backdrop-blur-[100px] rounded-[4px] p-0.5 items-center mx-auto shadow-sm whitespace-nowrap overflow-x-auto" 
              style={{ padding: "5px 2px", backdropFilter: "blur(100px)" }}>
              {navItems.map((item) => (
                item.isButton ? (
                  <Link
                    key={item.name}
            ...
            <div className="sm:hidden w-full max-w-[200px] mx-auto">
              <DropdownMenu open={isDropdownOpen} onOpenChange={setIsDropdownOpen}>
                <DropdownMenuTrigger asChild>
                  <button className="w-full flex items-center justify-between px-4 py-2 bg-[#17342C]/60 backdrop-blur-[100px] rounded-[4px] text-white">
                ...
                <DropdownMenuContent className="w-[200px] bg-[#8A9096]/60 backdrop-blur-[100px] border-0 text-white rounded-[4px]">
                  {navItems.map((item) => (
                    item.isButton ? (
                      <Link
                        key={item.name}
