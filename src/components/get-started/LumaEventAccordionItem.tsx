...
const LumaEventAccordionItem: React.FC<LumaEventAccordionItemProps> = ({ event, index }) => {
  ...
  return (
    ...
    >
      <AccordionTrigger className="py-6 px-6 hover:no-underline hover:bg-[#F8E3C8]/50 rounded-t-[15px]">
        <div className="flex flex-col items-start text-left w-full">
          <h4 className="font-caslon font-normal text-[28px] sm:text-[32px] text-[#674019]">{event.title}</h4>
          <div className="text-lg sm:text-xl text-[#674019]/60 font-medium font-uncut-sans">{timeDisplay}</div>
        </div>
      </AccordionTrigger>
      
      <AccordionContent className="px-6 pb-6 pt-0 bg-transparent rounded-b-[15px]">
        <div className="space-y-4">
          <p className="text-[#674019] mb-4">{event.description}</p>
          ...
          <Button 
            ...
            className="bg-[#674019] hover:bg-[#5D310E] text-white rounded-[50px] text-lg py-6 px-8 mt-4"
          >
